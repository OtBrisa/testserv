...
$main_content .= '<h1>Account Manager Bloqueado No Site.';
else
if(!$logged)
        if($action == "logout")
...
else
{
        if($action == "")
        {
                ...
                $account_vip = $account_logged->getPlayerVip_Time() ? '<b><font color="green"> VIP, '.$account_logged->getPlayerVip_Time().' Time left </font></b>' : '<b><font color="red">NOT VIP</font></b>';  
			...
			if($player->isLoaded()) {
				...
				if($account_logged->getId() == $player_account->getId()) {
					if($_POST['changecommentsave'] == 1) {
						...
						$player->setCustomField("show_skills", $new_showskills);
